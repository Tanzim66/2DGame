...
public class BFS {
    ...
    public Point[] findPath(final int[][] map,
        ...
        if (isOutOfMap(map, position.x, position.y) || isOutOfMap(map, destination.x, destination.y) || isBlocked(map, position.x, position.y) || isBlocked(map, destination.x, destination.y)) {
            return null;
        }
    ...
    private void lookAround(final int[][] map,
    	...
    	check.look(map, p.x + 1, p.y);
        check.look(map, p.x - 1, p.y);
        check.look(map, p.x, p.y + 1);
        check.look(map, p.x, p.y - 1);
    }
